...
public static class SetupProfileCatalogJson
{
    ...
    private static readonly HashSet<string> V1Properties = Set("id", "displayName", "description", "remoteName",
        "serviceUri", "webDavPathTemplate", "vendor", "defaultRemotePath", "defaultDriveLetter",
        "startWithWindowsByDefault", "mountArguments");
    private static readonly HashSet<string> V2Properties = Set("id", "displayName", "description", "defaultRemoteName",
        "connection", "inputs", "defaultRemotePath", "defaultDriveLetter", "startWithWindowsByDefault", "mountArguments");
    ...
    private static readonly HashSet<string> InputProperties = Set("key", "kind", "label", "helpText", "placeholder", "required");
    ...
    public static OperationResult<IReadOnlyList<SetupProfile>> Parse(string json)
    {
        ...
        try
        {
            ...
            foreach (var element in array.EnumerateArray())
            {
                ...
                var profile = schema == 1 ? ReadV1(element) : ReadV2(element);
    ...
    private static SetupProfile ReadV1(JsonElement element)
    {
        ValidateProperties(element, V1Properties, "profile");
        var service = ReadRequiredUri(element, "serviceUri");
        var vendor = ParseVendor(ReadRequiredString(element, "vendor", 128));
        return Common(element, "remoteName", new WebDavConnectionDefinition
        {
            BaseUrl = service,
            PathTemplate = ReadRequiredString(element, "webDavPathTemplate", 2_048),
            Vendor = vendor
        });
    }

    private static SetupProfile ReadV2(JsonElement element)
    {
        ValidateProperties(element, V2Properties, "profile");
        ...
        ValidateLegacyInputs(element);
        return Common(element, "defaultRemoteName", connection);
    }
    ...
    private static void ValidateLegacyInputs(JsonElement profile)
    {
        if (!profile.TryGetProperty("inputs", out var array)) return;
        if (array.ValueKind != JsonValueKind.Array)
            throw new JsonException("inputs must be an array.");
        if (array.GetArrayLength() is < 1 or > 8) throw new JsonException("inputs must contain between 1 and 8 fields.");
        var keys = new HashSet<string>(StringComparer.Ordinal);
        foreach (var element in array.EnumerateArray())
        {
            RequireObject(element, "input");
            ValidateProperties(element, InputProperties, "input");
            var key = ReadRequiredString(element, "key", 32);
            var kind = ReadRequiredString(element, "kind", 32);
            if (key is not "username" and not "password" || !keys.Add(key))
                throw new JsonException("Legacy inputs must contain unique username and password fields.");
            if (kind != (key == "password" ? "password" : "text") || !ReadOptionalBool(element, "required", true))
                throw new JsonException($"Legacy input '{key}' has incompatible metadata.");
            _ = ReadRequiredString(element, "label", 64);
            _ = ReadOptionalNullableString(element, "helpText", 256);
            _ = ReadOptionalNullableString(element, "placeholder", 128);
        }
        if (!keys.SetEquals(["username", "password"]))
            throw new JsonException("Legacy inputs must contain username and password fields.");
    }

    private static SetupProfile Common(JsonElement element, string remoteNameProperty,
        SetupConnectionDefinition connection)
    {
        ...
        return new SetupProfile
        {
            ...
            RemoteName = ReadRequiredString(element, remoteNameProperty, 128),
    ...
    private static string? ReadOptionalNullableString(JsonElement element, string name, int maximumLength) =>
        element.TryGetProperty(name, out _) ? ReadOptionalString(element, name, string.Empty, maximumLength) : null;
