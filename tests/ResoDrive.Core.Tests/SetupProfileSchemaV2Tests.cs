...
public sealed class SetupProfileSchemaV2Tests
{
    ...
    [Fact]
    public void ParseV1_ConvertsToTypedWebDav()
    {
        var result = SetupProfileCatalogJson.Parse(V1Json);

        Assert.True(result.Succeeded, result.Error?.Message);
        var profile = Assert.Single(result.Value!);
        Assert.IsType<WebDavConnectionDefinition>(profile.Connection);
        Assert.Equal("Legacy", profile.RemoteName);
    }
    ...
    [Fact]
    public void ParseV2_RejectsDuplicateInputSemantics()
    {
        var duplicate = WithLegacyInputs(WebDavJson).Replace(
            "{ \"key\": \"password\", \"kind\": \"password\", \"label\": \"App password\" }",
            "{ \"key\": \"username\", \"kind\": \"text\", \"label\": \"Other user\" }",
            StringComparison.Ordinal);

        var result = SetupProfileCatalogJson.Parse(duplicate);
    ...
    [Fact]
    public void ParseV2_RejectsWrongControlForSecret()
    {
        var result = SetupProfileCatalogJson.Parse(
            WithLegacyInputs(WebDavJson).Replace("\"kind\": \"password\"", "\"kind\": \"text\"", StringComparison.Ordinal));
    ...
    [Fact]
    public void ParseV2_RejectsOptionalCredentialFields()
    {
        var result = SetupProfileCatalogJson.Parse(
            WithLegacyInputs(WebDavJson).Replace("\"label\": \"Username\"", "\"label\": \"Username\", \"required\": false", StringComparison.Ordinal));
    ...
    [Fact]
    public void ParseV2_AcceptsDeprecatedInputMetadata()
    {
        var result = SetupProfileCatalogJson.Parse(WithLegacyInputs(WebDavJson));
    ...
    private static string WithLegacyInputs(string json) => json.Replace(
        "\"defaultDriveLetter\"",
        """
        "inputs": [
