...
public sealed class RcloneMountCoordinatorPendingTests
{
    ...
    [Fact]
    public void ReconnectPolicy_ZeroAttemptsMeansKeepTryingWithBoundedBackoff()
    {
        ...
        Assert.True(RcloneMountCoordinator.ShouldRestart(policy, 1));
        Assert.True(RcloneMountCoordinator.ShouldRestart(policy, 10_000));
        Assert.Equal(TimeSpan.FromSeconds(5), RcloneMountCoordinator.RestartDelay(policy, 1));
        Assert.Equal(TimeSpan.FromMinutes(5), RcloneMountCoordinator.RestartDelay(policy, 10_000));
    }

    [Fact]
    public void ReconnectPolicy_HonorsFiniteAttemptLimit()
    {
        var policy = new RestartPolicy { Enabled = true, MaximumAttempts = 3 };

        Assert.True(RcloneMountCoordinator.ShouldRestart(policy, 3));
        Assert.False(RcloneMountCoordinator.ShouldRestart(policy, 4));
    }
