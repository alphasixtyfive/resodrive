...
{
    [Fact]
    public void ToDomain_MigratesLegacySignInAutoMountPolicy()
    {
        var settings = new MountSettings
        {
            ...
            DisplayName = "Storage",
            RemoteName = "storage",
            AutoMount = "OnUserSignIn",
            Target = new MountTargetSettings { DriveLetter = 'R' }
        };

        var result = MountDefinitionMapper.ToDomain(settings);

        Assert.True(result.Succeeded);
        Assert.Equal(AutoMountPolicy.OnApplicationStart, result.Value?.AutoMount);
    }
    ...
    public void Mapping_PreservesConnectionHost()
    {
        var mapped = MountDefinitionMapper.ToDomain(
            ValidSettings() with { ConnectionHost = "cloud.example.com" });
