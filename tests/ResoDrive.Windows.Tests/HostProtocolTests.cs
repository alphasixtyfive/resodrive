...
{
    [Fact]
    public void AcceptsBaseDirectory_AllowsLegacyRequestsAndEquivalentPaths()
    {
        var current = Path.Combine(Path.GetTempPath(), "rdrive-install");

        Assert.True(HostProtocol.AcceptsBaseDirectory(null, current));
        Assert.True(HostProtocol.AcceptsBaseDirectory("  ", current));
